...
--2.
--The LAG function is used to show data from the preceding row or the table. When lining up rows the data is partitioned by country name and ordered by the data whn. 
--That means that only data from Italy is considered.
--Modify the query to show confirmed for the day before.

SELECT name, DAY(whn), confirmed,
LAG(confirmed) OVER (ORDER BY whn) pre
 FROM covid
WHERE name = 'Italy'
AND MONTH(whn) = 3
ORDER BY whn
...
SELECT name, DATE_FORMAT(whn,'%Y-%m-%d'),  confirmed - LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn) new
 FROM covid
WHERE name = 'Italy'
AND WEEKDAY(whn) = 0
ORDER BY whn
...
SELECT tw.name, DATE_FORMAT(tw.whn,'%Y-%m-%d'), tw.confirmed-lw.confirmed new
...
SELECT 
   world.name, ROUND(100000*confirmed/population,0),
   RANK() OVER (ORDER BY ROUND(100000*confirmed/population,0)) RK
...
FROM(
SELECT name, date, new, RANK() OVER (PARTITION BY name ORDER BY new DESC) rk
FROM(
SELECT name, DATE_FORMAT(whn,'%Y-%m-%d') date, confirmed, confirmed - LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn) new
FROM covid) c)d
WHERE rk = 1
AND new > 1000
ORDER BY 2
